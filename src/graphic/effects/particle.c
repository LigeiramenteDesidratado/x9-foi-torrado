...
private void __particle_init_textures(game_component_args* args) {
    ...
    for (int c = 0; c < MAX_COLORS; ++c) {
        for (int t = 0; t < MAX_SIZE; ++t) {
            ...
            texture_create(__texture_particle_array[c].textures[t], 2*radius, 2*radius, args);
            ...
            for(int y = 0; y < 2*radius; y++) {
                for(int x = 0; x < 2*radius; x++) {
                    ...
                    SDL_SetRenderDrawColor(renderer, game_colors[c].r, game_colors[c].g, game_colors[c].b, alpha);
            ...
            texture_reset_render_target(__texture_particle_array[c].textures[t], args);
...
int particle_ctor(particle_t* particle,
        float mid_x,
        float mid_y,
        int width,
        int height,
    ...
    if (entity_ctor(particle->particle, mid_x, mid_y, texture_get_w(particle->texture), texture_get_h(particle->texture)) != 0) {
        return -1;
    }
    ...
    switch (particle->dir) {
        case UP:
            if (middle.y <= 0) {
                ...
                float x = (float)SDL_sin(helper_deg2rad(particle->angle + particle->phase)) * particle->amp;
        ...
        case LEFT:
            if (middle.x <= 0) {
                ...
                float y = (float)SDL_sin(helper_deg2rad(particle->angle + particle->phase)) * particle->amp;
...
void particle_set_amp(particle_t* particle, int amp) {
    particle->amp = amp;
}
...
void particle_set_spd(particle_t* particle, int spd) {
    particle->spd = spd;
}
...
void particle_draw(read_only particle_t* particle, game_component_args* args) {
    texture_draw(particle->texture, entity_get_x(particle->particle), entity_get_y(particle->particle),  args);
}
