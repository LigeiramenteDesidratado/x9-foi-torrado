...
// Forward declaration
private
float __shake_decay(read_only shake_t *shake);
private
float __shake_noise(read_only shake_t *shake, int s);
private
float __shake_amplitude(read_only shake_t *shake);
...
int shake_ctor(shake_t *shake, int duration, int freq) {
  ...
  // Allocate and populate the samples array with randomized values between -1.0
  // and 1.0
...
float __shake_amplitude(read_only shake_t *shake) {
  ...
  return (float)((__shake_noise(shake, s0) +
                  (s - s0) *
                      (__shake_noise(shake, s1) - __shake_noise(shake, s0))) *
                 k);
}
