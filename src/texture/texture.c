...
int texture_ctor(UNUSED texture_t *texture) { return 0; }
...
void texture_load(texture_t *texture, const char *filename,
  ...
  SDL_QueryTexture(texture->p_text, NULL, NULL, &texture->area.w,
                   &texture->area.h);
}

void texture_create(texture_t *texture, int w, int h,
                    game_component_args *args) {

  if (texture->p_text != NULL) {
    SDL_SetError("Texture is not NULL");
  }
  texture->p_text = SDL_CreateTexture(graphic_get_renderer(args->graphic),
...
void texture_reset_render_target(UNUSED texture_t *texture,
                                 game_component_args *args) {
...
int texture_get_w(texture_t *texture) { return texture->area.w; }

int texture_get_h(texture_t *texture) { return texture->area.h; }

void texture_set_w(texture_t *texture, int w) { texture->area.w = w; }

void texture_set_h(texture_t *texture, int h) { texture->area.h = h; }
