...
// Behavior
void texture_draw(struct texture_t *texture, float x, float y,
                  game_component_args *args);
void texture_load(struct texture_t *texture, const char *filename,
                  game_component_args *args);
void texture_create(struct texture_t *texture, int w, int h,
                    game_component_args *args);
void texture_set_blend_mode(struct texture_t *texture);
void texture_set_color_mod(struct texture_t *texture, SDL_Color c);
void texture_set_render_target(struct texture_t *texture,
                               game_component_args *args);
void texture_reset_render_target(struct texture_t *texture,
                                 game_component_args *args);
