...
private
void __game_cap_frame_rate(long *then, float *remainder);
...
void game_ctor(game_t *game) {
  ...
  game->pallete = SDL_AllocPalette(MAX_COLORS);
  SDL_SetPaletteColors(game->pallete, game_colors, 0, MAX_COLORS);
}
...
int game_main_loop(game_t *game) {
  ...
  while (input_quit_signal(game->input)) {
    ...
    text_draw(game->text, game->args, 10,
              screen_get_window_w(game->screen) - 10, 10, 0x00, 0xff, 0x00,
              "FPS: %.2f", elapsed);

    text_draw(game->text, game->args, rect.x + 10, rect.w - 10, rect.y + 10,
              0xff, 0x00, 0x00, "A E S T H E T I C__");
