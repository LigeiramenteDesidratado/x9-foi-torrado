...
int player_ctor(player_t *player, int area_w, int area_h, float start_x,
  ...
  if (entity_ctor(player->pneu, start_x, start_y,
                  texture_get_w(player->texture),
                  texture_get_h(player->texture)) != 0) {
    return -1;
  }
...
void player_draw(read_only player_t *player, game_component_args *args) {
  ...
  texture_draw(player->texture, entity_get_x(player->pneu),
               entity_get_y(player->pneu), args);
}
...
void player_do(player_t *player, game_component_args *args) {
  ...
  if (input_scan_key(args->input, SDL_SCANCODE_J)) {
    if (entity_get_y(player->pneu) <
        (player->area_h - entity_get_h(player->pneu))) {
      entity_set_dy(player->pneu, PLAYER_SPEED);
    }
  }
  ...
  if (input_scan_key(args->input, SDL_SCANCODE_L)) {
    if (entity_get_x(player->pneu) <
        (player->area_w - entity_get_w(player->pneu))) {
      entity_set_dx(player->pneu, PLAYER_SPEED);
    }
  }
